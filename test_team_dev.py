...
def create_team(gearman_worker, gearman_job):
    ...
    new_group.create(data[0], mail_enabled=True, security_enabled=True, owners=[developer_id],
                    members=members_list[0:10], description=data[1], mail_nickname=data[2])

    # group_data = new_group.get()
    # newgroup_id = new_group.get_id()
    ...
    #gearman_worker.send_job_data(gearman_job, new_group.team_id)
...
def get_team_data(team_id="955529db-5622-4dca-9c90-8cb0e5fe032f"):
    ...
    #print(json.loads(response.texts))
